describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItem = await page.$('product-item'); // grab the item
    let shadowRoot = await prodItem.getProperty("shadowRoot"); // grab the item's shadowRoot
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      let shadowRoot = await prodItems[i].getProperty("shadowRoot"); // grab the item's shadowRoot
      let button = await shadowRoot.$("button"); // grab the shadowRoot's button
      let text = await button.getProperty("innerHTML");
      expect(text["_remoteObject"].value).toBe("Remove from Cart");
    }
